...
5. Покажи короткий список homework со статусом `статус:ожидает`.
...
```bash
gh issue edit ISSUE_NUMBER --remove-label "статус:ожидает" --add-label "статус:заблокировано"
...
- `статус:ожидает` - домашка выдана;
- `статус:в-работе` - ученик начал;
